...
const TransactionSelectMaterial = () => {
  ...
  return (
    <div>
      ...
      <h1 className="text-2xl font-bold text-gray-800 mb-4">Raw Materials</h1>
      <div className="bg-white rounded-xl shadow-sm border overflow-hidden">
        <table className="w-full text-sm">
          <thead>
            <tr className="bg-gray-50 text-left">
              <th className="px-6 py-3 text-[hsl(174,60%,30%)] font-semibold">#</th>
              <th className="px-6 py-3 text-[hsl(174,60%,30%)] font-semibold">Raw Material Name</th>
            </tr>
          </thead>
          <tbody>
            {pageItems.map((r, i) => (
              <tr key={r.id} className={`cursor-pointer ${i % 2 === 0 ? "bg-[hsl(174,40%,97%)]" : "bg-white"} hover:bg-[hsl(174,40%,93%)]`} onClick={() => navigate(`/dashboard/rawmaterial/transaction/${r.id}`)}>
                <td className="px-6 py-3">{(page - 1) * PAGE_SIZE + i + 1}</td>
                <td className="px-6 py-3">{r.name}</td>
              </tr>
            ))}
            {items.length === 0 && (
              <tr><td colSpan={2} className="px-6 py-8 text-center text-gray-400">No raw materials. Create some first.</td></tr>
            )}
          </tbody>
        </table>
      </div>
...
export default TransactionSelectMaterial;
